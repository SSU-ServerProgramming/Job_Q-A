...
struct FinalRejectionPopupView: View {
    ...
    var body: some View {
        VStack {
            ...
            if selectedApplicants.count > 1 {
                Text("\(firstApplicantName) 님 외 \(selectedApplicants.count - 1)명의 상태를 '최종 불합격'으로 변경할까요?")
                    .padding()
                    .font(.system(size: 16))
            } else {
                Text("\(firstApplicantName) 님의 상태를 '최종 불합격'으로 변경할까요?")
                    .padding()
                    .font(.system(size: 16))
            }
